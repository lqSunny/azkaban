...
public class Trigger implements Runnable {
  ...
  private static final Logger logger = Logger.getLogger(com.datapps.zebra.workflow.execapp.Trigger.class);
      ...
      for (final TriggerAction action : this.actions) {
        try {
          ...
          logger.error("Failed to do action " + action.getDescription()
              + " for execution " + com.datapps.zebra.workflow.execapp.Trigger.this.execId, e);
        }
      }
    }
  }
